 ...
 * @author inta
 * @date 2020/3/24
 * @describe 珠玑妙算游戏（the game of master mind）的玩法如下。
 *
    ...
    public int[] masterMind(String solution, String guess) {
        ...
        for (int i = 0; i < 4; i++) {
            if (solution.charAt(i) == guess.charAt(i)) {
                bingo ++;
                counts[solution.charAt(i)] --;
                visited[i] = true;
            }
        }
        for (int j = 0; j < 4; j++) {
            if (!visited[j] && counts[guess.charAt(j)] > 0) {
                wrong ++;
                counts[guess.charAt(j)] --;
            }
        }
        return new int[]{bingo, wrong};
    }
}
