 ...
 * @author inta
 * @date 2020/7/29
 * @describe 我们得到了一副藏宝图，藏宝图显示，在一个迷宫中存在着未被世人发现的宝藏。
 *
 ...
 *     我们需要继续回去 O 搬石头。 O->第三行的M, cost = 4, 此时所有机关均触发 第三行的M->T, cost = 2，去T点拿宝藏。 总步数为16。 图片.gif
 *
 * 示例 2：
 *
 *     输入： ["S#O", "M.#", "M.T"]
 ...
 *
 */
public class Q13minimalSteps {
/*    public int minimalSteps(String[] maze) {
        int m = maze.length;
        int n = maze[0].length();
        int[][] map = new int[m][n];
        int[] start = new int[2], end = new int[2];
        List<int[]> m_lists = new ArrayList<>();
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
...
//                    map[i][j] = 0;
        ...
        //此时，我们获得了一个map，保存了墙壁1的信息，以及一个List集合m_lists保存了所有机关索引，起点start和终点end
