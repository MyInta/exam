 ...
 * @author inta
 * @date 2020/10/24
 * @describe 你将会获得一系列视频片段，这些片段来自于一项持续时长为 T 秒的体育赛事。
 * 这些片段可能有所重叠，也可能长度不一。
 *
 * 视频片段 clips[i] 都用区间进行表示：开始于 clips[i][0] 并于 clips[i][1] 结束。
 * 我们甚至可以对这些片段自由地再剪辑，例如片段 [0, 7] 可以剪切成 [0, 1] + [1, 3] + [3, 7]
 * 三部分。
 *
...
public class Q1024videoStitching {
    ...
    public int videoStitching(int[][] clips, int T) {
        ...
        int left = clips[0][0], right = clips[0][1], res = 1;
        if (left != 0) return -1;
        for (int[] clip : clips) {
            if (clip[1] > right) {
                if (clip[0] > right) return -1;
                if (clip[0] > left) {
                    left = right;
                    res++;
                }
