 ...
 * @author inta
 * @date 2021/1/31
 * @describe Alice 和 Bob 共有一个无向图，其中包含 n 个节点和 3  种类型的边：
 *
 * 类型 1：只能由 Alice 遍历。
 * 类型 2：只能由 Bob 遍历。
 ...
 */
public class Q1579maxNumEdgesToRemove {
    // 保持两个并查集，都连通情况下，考虑边数
    public int maxNumEdgesToRemove(int n, int[][] edges) {
        ...
        for (int[] edge : edges) {
            if (edge[0] == 3) {
                if (unionFindAlice.merge(edge[1], edge[2])) {
                    unionFindBob.merge(edge[1], edge[2]);
                } else {
                    res++;
                }
            }
        ...
        for (int[] edge : edges) {
            if (edge[0] == 1) {
                ...
                if (!unionFindBob.merge(edge[1], edge[2])) {
                    res++;
                }
            }

        }
        if (unionFindAlice.getCount() != 2 || unionFindBob.getCount() != 2) {
            return -1;
        }
        return res;
    }
