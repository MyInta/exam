 ...
 * @author inta
 * @date 2021/6/1
 * @describe 给你一个下标从 0 开始的正整数数组 candiesCount ，其中 candiesCount[i] 表示你拥有的第 i 类糖果的数目。
 ...
 * 1 <= dailyCapi <= 10^9
 */
public class Q1744canEat {
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        int len = candiesCount.length;
        long[] counts = new long[len + 1];
        counts[1] = candiesCount[0];
        for (int i = 2; i <= len; i++) {
            counts[i] = counts[i - 1] + candiesCount[i - 1];
        }
        int size = queries.length;
        boolean[] res = new boolean[size];
        int index = 0;
        for (int[] query : queries) {
            if (counts[query[0]] < (long)query[1] * query[2] && counts[query[0] + 1] >= query[1]) {
                res[index] = true;
            } else {
                res[index] = false;
            }
