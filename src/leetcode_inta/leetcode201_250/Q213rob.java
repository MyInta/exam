...
public class Q213rob {
    ...
    public int rob(int[] nums) {
        ...
        int[] firstDp = new int[len - 1];
        firstDp[0] = nums[0];
        // 最后一家不能取得情况下，第一家和第二家还是需要考虑是否选取哪一个的
        firstDp[1] = nums[0];
        for (int i = 2; i < len - 1; i++) {
            firstDp[i] = Math.max(firstDp[i - 1], firstDp[i - 2] + nums[i]);
        }
        ...
        secondDp[1] = nums[1];
        // 如果不取第二家也是有可能的
        secondDp[2] = Math.max(nums[1], nums[2]);
        for (int j = 3; j < len; j++) {
            secondDp[j] = Math.max(secondDp[j - 1], secondDp[j - 2] + nums[j]);
        }
        // 第一种dp选择了偷第一家，所以最后一家不能取，dp选取len-2；第二种最后一家可取，dp选择len-1
