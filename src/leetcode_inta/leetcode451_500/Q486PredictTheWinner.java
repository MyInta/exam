 ...
 * @author inta
 * @date 2020/9/1
 * @describe 给定一个表示分数的非负整数数组。 玩家 1 从数组任意一端拿取一个分数，
...
public class Q486PredictTheWinner {
    //思想：偶数情况，先选的可以控制自己选偶数组或者奇数组，数值相等也算其赢，故此情况必赢
    //当奇数时，使用dp[i][j]统计i->j赚取的分数差值，每次选择左或右，即num[i] - dp[i+1][j]或num[j] - dp[i][j-1]
    public boolean PredictTheWinner(int[] nums) {
        if (nums.length % 2 == 0) return true;
        int[][] dp = new int[nums.length][nums.length];
        //找到递归式后，我们只需要考虑从单个到多个，并且考虑从左到右累积即可
        //k表示，选取的数组片段添加的长度
        ...
        //递归完之后，考虑最后的片段dp[i][j]统计出来的分值是否符合要求
